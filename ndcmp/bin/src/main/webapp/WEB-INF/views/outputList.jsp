			...
			<tbody>
				<c:forEach items="${outputList }" var="output">
					<tr>
						...
						<td><c:out value="${output.description}" /></td>
						<td>
						<%-- <c:if test="${COU_EDT == true }"> --%>
						<a href="editOutput?OutputId=<c:out value='${output.id}'/>">
								<img src="<c:url value="/static/images/edit.png" />" alt="Edit"
								title="Edit Output"></a><%-- </c:if>  --%>
						<%-- <c:if test="${COU_DEL == true }"> --%>
						<a href="#" data-toggle="modal" data-target="#myModal" data-id=${output.id } data-href="deleteOutput?themeId=${output.id }"> 
									<img src="<c:url value="/static/images/delete.png" />" alt="Delete"	title="Delete Output"></a>
						<%-- </c:if> --%></td>
					</tr>
				</c:forEach>
			</tbody>
